...
public class ProcessDefinitionServiceImpl implements IProcessDefinitionService {
	...
	@Override
	public TableDataInfo<ProcessDefinitionDTO> selectPageProcessDefinitionList(ProcessDefinitionDTO processDefinition) {
		...
		List<ProcessDefinition> processDefinitions = processDefinitionQuery.listPage(((int)PageUtils.buildPage().getCurrent()-1)*(int)PageUtils.buildPage().getSize(),(int)PageUtils.buildPage().getSize());
		long count = processDefinitionQuery.count();
		page.setTotal(count);
		if (count!=0) {
			Set<String> ids = processDefinitions.parallelStream().map(ProcessDefinition::getDeploymentId).collect(Collectors.toSet());
			List<ActReDeploymentVO> actReDeploymentVOS = actReDeploymentMapper.selectBatchIds(ids);
			...
			page.setRecords(processDefinitionDTOS);
		}
