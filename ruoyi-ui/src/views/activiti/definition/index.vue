...
<script>
import {listDefinition, delDefinition, suspendOrActiveApply, getProcessDefineXML} from "@/api/activiti/definition";
...
export default {
  ...
  methods: {
    ...
    OnlineDrawingProcess() {
      this.$router.push('/activity/bpmn/index')
    },
    OnlineModificationProcess(data) {
      getProcessDefineXML({'deploymentId':data.deploymentId, 'resourceName': encodeURI(data.resourceName)}).then(response => {
        let param = {xmlString:response,processId:data.id,processName:data.name}
        this.$router.push({path:'/activity/bpmn/index', query: param})
      })
    },
