...
<script>
import translations from "@/views/activiti/bpmn/translations"
// 自定义渲染（隐藏了 label 标签）
import CustomRenderer from "@/views/activiti/bpmn/modules/custom-renderer";
// 自定义元素选中时的弹出菜单（修改 默认任务 为 用户任务）
import CustomContentPadProvider from "@/views/activiti/bpmn/package/process-designer/plugins/content-pad";
// 自定义左侧菜单（修改 默认任务 为 用户任务）
import CustomPaletteProvider from "@/views/activiti/bpmn/package/process-designer/plugins/palette";
...
export default {
  ...
  created() {
    this.xmlString = this.$route.query.xmlString || ''
    this.controlForm.processId = this.$route.query.processId || ''
    this.controlForm.processName = this.$route.query.processName || ''
  },
  methods: {
    initModeler(modeler) {
      setTimeout(() => {
